...
const shouldAllocateCredits = (dbUser:any, currentPlan:string) => {
  // Always allocate if plan changed     
  //if user has purched new plan
  if (dbUser.currentPlan !== currentPlan) return true;

  // Allocate if never allocated before 
  //if user has just signup
  if (!dbUser.creditsLastAllocatedAt) return true;

  // Allocate if it's a new calendar month since last allocation
  //if user has already allocated credits but it's a new month, then allocate credits again based on their current plan
  const now = new Date();
